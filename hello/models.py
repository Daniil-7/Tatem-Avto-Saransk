...
CONDITIONS = (("Новый", "Новый"), ("Б/У", "Б/У"))


cloudinary.config(
    cloud_name="name cloud key", api_key="api key", api_secret="password key"
)
...
class Product(models.Model):
    ...
    condition = models.CharField(
        choices=CONDITIONS, max_length=200, verbose_name="Состояние:", default="Новый"
    )
    image_product1 = cloudinary.models.CloudinaryField(
        verbose_name="Фото товара(обязательно):"
    )
    image_product2 = cloudinary.models.CloudinaryField(
        verbose_name="Второе фото товара(необязательно):", blank=True
    )
    image_product3 = cloudinary.models.CloudinaryField(
        verbose_name="Третье фото товара(необязательно):", blank=True
    )
